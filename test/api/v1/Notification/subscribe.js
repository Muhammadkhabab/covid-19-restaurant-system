	...
	describe('POST /notifications (subscribe)', () => {
		...
		it('1. should return error because request of the body was incorrect (had missing fields)', (done) => {
			...
			request(app)
				...
				.end((err, res) => {
					...
					expect(res.body.errors[0]).to.have.property('msg').to.equal('Missing contact!');
					expect(res.body.errors[0]).to.have.property('param').to.equal('contact');
					done();
				});
		});
		...
		it('2. should return error because start date of subscription in the past', (done) => {
			...
			request(app)
				...
				.end((err, res) => {
					...
					expect(res.body).to.have.property('errors');
					expect(res.body.errors).to.have.lengthOf(1);
					expect(res.body.errors[0]).to.have.property('msg').to.equal('Start time cannot be in the past!');
					done();
				});
		...
		it('3. should return error because end date of subscription before start date', (done) => {
			const notification3 = {
				...
				start_time: '2021-04-21T11:12:15.291Z',
				end_time: '2021-04-20T11:12:15.291Z',
				max_distance: 25,
				min_table_distance: 4,
				cuisine: 'American',
				dine_in: true
			...
			request(app)
				...
				.end((err, res) => {
					...
					expect(res.body).to.have.property('errors');
					expect(res.body.errors).to.have.lengthOf(1);
					expect(res.body.errors[0]).to.have.property('msg').to.equal('End time must be later than start time!');
					done();
				});
		});
		...
		it('4. should return errors because there were multiple missing fields', (done) => {
			...
			request(app)
				...
				.end((err, res) => {
					...
					const msgs = [
						['is_contact_email', 'Missing information about if contact is email or phone number!'],
		...
		it('5. should return error because user did not pick from dine_in, dine_outside, pickup, curbside, or delivery', (done) => {
			...
			request(app)
				...
				.end((err, res) => {
					...
					expect(res.body).to.have.property('errors');
					expect(res.body.errors).to.have.lengthOf(1);
					expect(res.body.errors[0]).to.have.property('msg').to.equal('Enter at least one option for dining experience!');
					done();
				});
		});
		...
		it('6. should return the notification when notification is created!', (done) => {
			...
			request(app)
				...
				.end((err, res) => {
					...
					expect(res.body).to.have.property('is_contact_email').to.equal(is_contact_email);
					...
					expect(res.body).to.have.property('min_table_distance').to.equal(min_table_distance);
					expect(res.body).to.have.property('cuisine').to.equal(cuisine);
					expect(res.body).to.have.property('dine_in').to.equal(dine_in);
					expect(res.body).to.have.property('_id'); done();
				});
		});
	});
}